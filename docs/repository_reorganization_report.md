...
| `879e0de` | 验证重构结果数值等价 | 新增等价脚本、最终验证报告，并修复兼容提示的 Windows 终端编码问题。 |
| `6d0375d` | 校正重构报告验证数据 | 校正测试计数以及成本和节省率的展示精度。 |
| `8fcbeb0` | 拒绝链接型最终输出路径 | 阻止最终输出入口沿符号链接或 Windows 目录联接覆盖外部目标。 |
| 本次验收修正提交 | 强化最终验收与修正记录 | 拒绝等价结果中的非有限数值和文本、布尔缺失，并覆盖完整提交范围的差异检查。 |
...
| 最终输出路径本身若为符号链接、断链或 Windows 目录联接，先调用 `resolve()` 会跟随到外部目标，发布时可能替换并删除目标原内容。 | 在解析前对调用者传入的最终目录项执行 `lstat()`，明确拒绝符号链接、断链和目录联接；只解析父目录，并对父目录下未经解引用的最终目录项执行发布。 | 最终输出入口属于事务边界，不得通过链接把发布范围扩展到外部目录。 | 真实 Windows 目录联接回归先复现受保护标记被替换，修复后抛出明确 `ValueError` 且目标保持不变；普通和断链符号链接测试在当前账户无创建权限时跳过。 | 0；只强化最终输出路径安全。 |
...
| 重组后缺少可重复执行的逐列数值证明，人工查看总成本不足以发现列顺序、文本、行数或局部数值变化。 | 新增 `scripts/verify_reorganization_equivalence.py`，显式选择 `legacy` 或 `current` 布局；列名和顺序、行数、文本和布尔值精确比较，非计时数值使用 `atol=1e-9`、`rtol=0`，四个明确计时字段只要求有限且非负。 | 将重组“不改变实验结果”转化为自动化、可失败的验收条件，并可复用于正式入口和兼容入口。 | 脚本测试覆盖容差内差异、当前布局互比、列顺序、行数、文本、超容差数值、负计时和无限计时。 | 0；验证工具不参与求解。 |
| 等价脚本最初允许 `equal_nan=True` 且未预先检查有限性，会把两侧同位置的 `NaN` 或同号无穷误判为等价；文本和布尔列的成对缺失也可能通过。 | reference 与 actual 的所有非计时数值列分别要求全部有限，再以 `equal_nan=False` 比较；文本和布尔列分别拒绝任意缺失值。 | 等价验收必须证明结果是有效有限数据，而不只是两侧具有相同的无效表示。 | 新增成对 `NaN`、成对无穷、单侧非有限值、文本缺失和布尔缺失拒绝测试；旧实现 RED，修复后 GREEN。 | 0；只强化验收工具，不参与求解。 |
...
四组算例状态与运行成本如下。展示值均由正式 CSV 原值四舍五入：成本保留 2 位小数，节省率保留 6 位小数；数值等价验证使用未舍入的 CSV 原值。

| 算例 | 状态 | 运行成本（CNY） | 相对 `renewables_only` 节省率 |
| --- | --- | ---: | ---: |
| `renewables_only` | `optimal` | 1,137,926.26 | 0.000000% |
| `renewables_shift` | `optimal` | 1,108,096.33 | 2.621429% |
| `renewables_storage` | `optimal` | 1,123,755.82 | 1.245287% |
| `joint` | `optimal` | 1,094,637.05 | 3.804219% |
...
- 正式测试：共运行 105 项，其中 101 项通过，4 项因当前 Windows 账户没有创建符号链接权限而跳过；对应的 Windows 目录联接保护测试实际运行并通过。
- 归档测试：11 项通过。
- 正式测试总数由重组完成时的 91 项增加为 105 项：新增 5 项初始等价检查、1 项终端编码、3 项最终输出入口安全及 5 项非有限值或缺失值拒绝测试。
- `compileall` 覆盖 `dc_energy_opt`、`scripts` 和两个入口。
- `pip check` 验证环境依赖关系。
- `git diff --check dda43b3f79f2a221cb5b5d9d3e07187a74255c2e..HEAD` 验证本次重构全部已提交差异的格式。
