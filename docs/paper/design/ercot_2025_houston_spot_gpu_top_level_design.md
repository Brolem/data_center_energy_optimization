...
调度器按日滚动：对每个交割日，只使用该日已锁定的 DAM 价格，以及严格服从 48 小时信息保护期的研究者构造风、光与消费侧碳预测。实际风光、实际碳强度和未来 HP 占用只用于结果评价。整个月实际价格序列可用于事后计算成本，不能作为月初决策输入。
...
| EIA ERCO 风光实际发电与消费侧碳 | 研究者因果预测的历史训练，以及事后系统级匹配和排放评价 | 系统级信号，非 Houston 本地发电或边际碳；实际值不得泄漏到日前决策 |
...
| 门槛 | 通过标准 | 合理回退 |
| --- | --- | --- |
| 因果预测可得性 | 2024 固定滚动验证、48 小时保护和 2025 每日预测均可复现 | 若 Ridge 不优于中位数，只作为可复现情景输入，不宣称预测创新 |
...
这些门槛中，预测信息集与 GPU 功率映射都需要先核验；未验证前不能将其写成已完成的数据事实。

## 10. 两个月交付顺序

1. **第 1 周**：完成数据合同、2024 因果预测验证与年末上下文补齐；修正时间语义并重建 1,062 小时输入。
