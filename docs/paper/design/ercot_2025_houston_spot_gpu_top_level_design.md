...
3. 在前两项并列时，最小化 ERCOT 消费侧碳强度对应的排放。

“风光匹配”以系统级可再生能源出力的归一化/分位数暴露定义，并报告其 MWh 加权值；它不等价于数据中心直接消费了同等 MWh 的本地可再生电力。碳指标采用 EIA 已发布的消费侧强度，不表述为边际排放。
...
调度器按日滚动：对每个交割日，只使用该日已锁定的 DAM 价格，以及在预先登记的本地截止时刻之前发布的风光预测版本。实际风光、实际碳强度和未来 HP 预留只用于结果评价。整个月实际价格序列可用于事后计算成本，不能作为月初决策输入。
...
| EIA ERCO 风光实际发电与消费侧碳 | 事后系统级匹配和排放评价 | 系统级信号，非 Houston 本地发电或边际碳 |
